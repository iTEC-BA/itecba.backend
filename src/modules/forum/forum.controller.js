...
const ADJECTIVES = ["Valiente", "Curioso", "Brillante", "Furioso", "Sereno", "Audaz", "Veloz", "Sabio", "Astuto", "Tenaz", "Ágil", "Firme", "Perspicaz", "Osado", "Prudente", "Ingenioso", "Reflexivo", "Agudo", "Directo", "Lúcido"];
const NOUNS = ["Panda", "Águila", "Tigre", "Lobo", "Zorro", "Oso", "Halcón", "Lince", "Búho", "Cóndor", "Jaguar", "Pantera", "Delfín", "Cuervo", "Mapache", "Nutria", "Alce", "Castor", "Coyote", "Murciélago"];
...
const BAD_WORDS = ["pelotudo", "boludo", "forro", "idiota", "imbecil", "estupido", "tarado", "cretino", "inutil", "hdp", "mierda", "puto", "puta", "carajo"];
...
export const getPosts = async (req, res, next) => {
  try {
    ...
    const { rows } = await turso.execute({
      sql: `
        SELECT p.id, p.pseudonym, p.body, p.upvotes, p.created_at,
    ...
    res.status(200).json({
      posts: rows, total: Number(countRows[0].total), page, pageSize: PAGE_SIZE,
...
export const createPost = async (req, res, next) => {
  try {
    ...
    if (!body?.trim() || body.trim().length < 3) return res.status(400).json({ error: "El contenido debe tener al menos 3 caracteres." });
    if (body.trim().length > 1000) return res.status(400).json({ error: "El contenido no puede superar los 1000 caracteres." });
    if (containsBadWords(body)) return res.status(400).json({ error: "Publicación rechazada por vocabulario inapropiado." });
    ...
    res.status(201).json({ message: "Publicado con éxito", id: Number(lastInsertRowid), pseudonym, body: body.trim(), upvotes: 0, reply_count: 0, created_at: new Date().toISOString() });
...
export const createReply = async (req, res, next) => {
  try {
    ...
    if (!body?.trim() || body.trim().length < 3) return next(badRequest("La respuesta debe tener al menos 3 caracteres."));
    if (body.trim().length > 1000) return next(badRequest("La respuesta no puede superar los 1000 caracteres."));
    if (containsBadWords(body)) return next(badRequest("Tu respuesta contiene lenguaje inapropiado."));
    ...
    // ── WEB PUSH: NOTIFICAR AL AUTOR ORIGINAL ──
    ...
    if (parentUserHash !== userHash) {
      ...
      } catch (_e) { /* Fallo silencioso de push */ }
    }

    res.status(201).json({ id: Number(lastInsertRowid), parent_id: Number(id), pseudonym, body: body.trim(), upvotes: 0, created_at: new Date().toISOString(), reply_count: 0 });
  } catch (err) { next(err); }
};
...
export const deletePost = async (req, res, next) => {
  try {
    ...
    if (rows[0].user_hash !== userHash && req.user.role !== "admin") return res.status(403).json({ error: true, message: "No podés eliminar este post" });
    await turso.execute({ sql: `DELETE FROM anonymous_posts WHERE id = ?1`, args: [id] });
    res.status(200).json({ message: "Post eliminado" });
  } catch (err) { next(err); }
};

// ── CONTROLADORES DE PUSH NOTIFICATIONS ────────────────────────────────────────
export const savePushSubscription = async (req, res, next) => {
  try {
    ...
    await turso.execute({
      sql: `INSERT INTO push_subscriptions (user_hash, subscription, updated_at) VALUES (?1, ?2, ?3) ON CONFLICT(user_hash) DO UPDATE SET subscription = excluded.subscription, updated_at = ?3`,
